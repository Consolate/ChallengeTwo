#creating filestorage file 
cusnames=[]
cuspassword=[]
cusbalance=[]

#openeing the storage file to collect customer old data
namefile=open("cusnamefile.txt","r")
passfile=open("cuspassfile.txt","r")
balfile=open("cusbalfile.txt","r")

# put data from the storage files into the empty list
#check list of customer names
for line in filename:
    cusnames.append(line[:-1])
    passfile.close()

#check list of customer balance
for line in balfile:
...
while name not in cusnames and len(name)>3:
    name=input("Please type in your name to create a new bank account\n")
    if name not in cusnames:
        cusnames.append(name)
        filewrite(cusnames)
        break
    print("Sorry, name already in use ")
    ans=input("Are you already a member of this bank? (y/n)")
    if ans.lower()=='y':
        oldcuscheck()
    else:
            cusaccountcheck()
